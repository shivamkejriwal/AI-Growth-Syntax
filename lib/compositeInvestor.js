...
import { CongressionalClient } from './congressionalClient.js';
import { InstitutionalHoldingsClient } from './institutionalHoldingsClient.js';
...
import { defaultMockDataManager } from './mockDataManager.js';
import { defaultSignalsEngine, SignalsEngine } from './signalsEngine.js';
...
export class CompositeInvestor {
  ...
  constructor(options = {}) {
    ...
    this.congressional = new CongressionalClient(options.congressional);
    this.institutional = new InstitutionalHoldingsClient(options.institutional);
  }
  ...
  async getStandardCompanyData(ticker, options = {}) {
    ...
    // In mock mode, pull mock data directly from SQL (SQLite) or Firestore
    if (options.mode === 'mock') {
      const mockData = await defaultMockDataManager.getMockCompanyData(sym, this.store.db);
      if (mockData) {
        return mockData;
      }
    }
  ...
  async buildStandardCompanyData(ticker, options = {}) {
    ...
    // 4. Ingest Scuttlebutt, Competitors, Congressional Trades & Institutional Holdings
    const [competitorAnalysis, seekingAlphaFeed, ddgAudit, congressionalTrades, institutionalHoldings] = await Promise.all([
      ...
      this.congressional.getCongressionalTrades(sym, 15).catch(() => ({})),
      this.institutional.getInstitutionalHoldings(sym, 15).catch(() => ({}))
    ]);
  ...
  async getCompositeDossier(ticker, options = {}) {
    ...
    // 3. Compute Complete Equity Visualizations (Snowflake Radar, DCF Intrinsic, Sankey Flows, Health Lines)
    const equityVisuals = calculateEquityVisuals(
      companyData.financials || {},
      companyData.macroEnvironment || {},
      inference.experts?.aswathDamodaran?.costOfCapital || {},
      inference.experts?.peterLynch || {},
      inference.experts?.warrenBuffett?.ownerEarningsAnalysis || {},
      inference.experts?.warrenBuffett?.retainedEarningsTest || {},
      inference.experts?.aswathDamodaran?.rndNormalization || {}
    );

    // 4. Derive Investment & Research Signals (14 Risk Checks + Custom Signals)
    const signals = defaultSignalsEngine.evaluateAll(companyData, options);

    // 5. Synthesize into composite research dossier
    return {
      ...
      competitorAnalysis: companyData.competitors?.peerComparison || [],
      macroContext: companyData.macroEnvironment,
      experts: inference.experts,
      synthesis: inference.synthesis,
      grahamValuation: inference.experts?.benjaminGraham,
      rawFinancials: companyData.financials,
      secFilings: companyData.secFilings,
      marketIntelligence: companyData.marketIntelligence,
      congressionalTrading: companyData.congressionalTrading,
      institutionalOwnership: companyData.institutionalOwnership,
  ...
  async generateCompositeDossier(ticker, options = {}) {
    ...
    const dossier = {
      ...
      signals: defaultSignalsEngine.evaluateAll({
        profile: { ticker: sym, name: financials.name },
        market: {
          ...
          dividendYieldPercent: (financials.dividendYield || 0) * 100
        },
        financials: {
          latest: financials.latest || {},
          annualHistory: (financials.sortedYears || []).map(y => ({ fiscalYear: y, ...(financials.yearsMap?.[y] || {}) })),
          ratios: { debtToEquity: financials.debtToEquity, dividendPayoutRatio: financials.payoutRatio }
        },
        secFilings: edgarFilings,
        marketIntelligence: scuttlebuttData
      }, options),
      get riskChecks() { return this.signals?.riskChecks || []; }
    };
   ...
   */
  async generateMacroDossier(options = {}) {
    if (options.mode === 'mock') {
      const mockMacro = await defaultMockDataManager.getMockMacro(this.store.db);
      if (mockMacro) return mockMacro;
    }
