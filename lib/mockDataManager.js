...
import { DEMO_DATASETS, DEMO_SPDR_SECTOR_ETFS, DEMO_SECTOR_PERFORMANCE } from './demoData.js';
...
export class MockDataManager {
  ...
  async ensureMockDataSeeded(db = this.db) {
    ...
    const status = db.getStale ? db.getStale(checkKey, namespace) : await db.get(checkKey, namespace);
  ...
  async getMockCompanyData(ticker, db = this.db) {
    ...
    let data = db.getStale ? db.getStale(key, namespace) : await db.get(key, namespace);
    if (!data) {
      await this.ensureMockDataSeeded(db);
      data = db.getStale ? db.getStale(key, namespace) : await db.get(key, namespace);
    }
