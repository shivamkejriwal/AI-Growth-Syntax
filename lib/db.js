 ...
 */
/**
 * Helper to discover a valid Firebase/Google Cloud access token.
 */
export async function getFirebaseAccessToken() {
  if (process.env.FIREBASE_TOKEN) return process.env.FIREBASE_TOKEN;
  if (process.env.GCLOUD_TOKEN) return process.env.GCLOUD_TOKEN;
  ...
  try {
    ...
    if (fs.existsSync(configPath)) {
      ...
      try {
        ...
        const fbAuth = require('/usr/local/lib/node_modules/firebase-tools/lib/auth.js');
        const tok = await fbAuth.getAccessToken(conf.tokens?.refresh_token, conf.tokens?.scopes || []);
        if (tok?.access_token) return tok.access_token;
        if (typeof tok === 'string') return tok;
      } catch {}
      if (conf.tokens?.access_token) return conf.tokens.access_token;
    }
  } catch {}
  return null;
}
...
/**
 * Cloud Firestore Database Store for Deployed / Firebase Mode.
 * Supports native `@google-cloud/firestore` SDK and lightweight authenticated REST fallback.
 */
export class FirestoreStore {
  ...
  async _ensureInit() {
    ...
    if (!this.firestore && !this.authToken) {
      this.authToken = await getFirebaseAccessToken();
    }
  }
  ...
  async get(key, namespace = 'default') {
    ...
    if (this.firestore) {
      ...
      try {
        const url = `https://firestore.googleapis.com/v1/projects/${this.projectId}/databases/(default)/documents/${this.collectionName}/${docId}`;
        const res = await fetch(url, { headers: { Authorization: `Bearer ${this.authToken}` } });
        if (res.ok) {
          const doc = await res.json();
          const fields = doc.fields || {};
          if (fields.date_str?.stringValue !== today) return null;
          if (fields.data?.stringValue) return JSON.parse(fields.data.stringValue);
        }
      } catch (err) {
        console.warn(`[FirestoreStore REST] Error reading ${docId}:`, err.message);
      }
    }
  ...
  async set(key, data, namespace = 'default', dateStr = getTodayDateStr()) {
    ...
    if (this.firestore) {
      ...
      try {
        ...
        console.warn(`[FirestoreStore REST] HTTP ${res.status} writing ${docId}`);
      } catch (err) {
        console.warn(`[FirestoreStore REST] Error writing ${docId}:`, err.message);
      }
    }
  ...
  async delete(key, namespace = 'default') {
    ...
    if (this.firestore) {
      ...
      try {
        const url = `https://firestore.googleapis.com/v1/projects/${this.projectId}/databases/(default)/documents/${this.collectionName}/${docId}`;
        await fetch(url, { method: 'DELETE', headers: { Authorization: `Bearer ${this.authToken}` } });
      } catch {}
    }
    this.memoryFallback.delete(`${namespace}:${key}`);
  }
