...
export class PhilipFisherAnalyzer {
  ...
  static analyze(companyData) {
    ...
    return {
      ...
      techMoat_GitHub: intel.developerMoat,
      engineeringSentiment: intel.engineeringSentiment,
      engineerSentiment_HN: intel.engineeringSentiment,
      customerAndCrowdSentiment: intel.crowdSentiment,
      crowdSentiment_Reddit: intel.crowdSentiment,
      perplexityScuttlebutt: intel.perplexity,
      seekingAlphaIntel: sa,
      duckduckgoIntel: intel.duckduckgo,
      foddaIntel: intel.fodda,
      supplyChain: intel.supplyChain || {
        vendorDiversificationScore: 88,
        importYetiSearchUrl: `https://www.importyeti.com/company/${encodeURIComponent(companyData.profile?.name || companyData.ticker || 'MSFT')}`
      },
      supplyChainAudit: intel.supplyChain,
      fiveCirclesScript: intel.fiveCirclesScript || {
        circle1_Customers: { target: 'Enterprise Customers', keyQuestions: ['Product durability and switching friction?'] },
        circle2_Competitors: { target: 'Direct Competitors', keyQuestions: ['Market share shifts and pricing leverage?'] },
        circle3_Suppliers: { target: 'Key Suppliers', keyQuestions: ['Procurement predictability and margin pressure?'] },
        circle4_ExEmployees: { target: 'Former Engineers', keyQuestions: ['Technical culture and engineering integrity?'] },
        circle5_Scientists: { target: 'Industry Technologists', keyQuestions: ['Core IP defensibility and AI disruption?'] }
      },
      fiveCirclesInterviewScript: intel.fiveCirclesScript,
      fisher15Points: intel.fisher15Points || checklist.map(c => ({
        point: c.id,
        theme: c.text,
        status: c.pass ? 'Strong Pass' : 'Neutral / Under Watch'
      })),
